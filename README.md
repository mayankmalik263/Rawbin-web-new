...
## 📜 Project History & Git Milestones (Last 91 Commits)

The development of the Rawbin website has progressed through key milestones across the repository's 91 commits:

### Phase 1: Foundation & Sticky Navigation
*   **Commits:** `1cf69b6` to `778768e`
*   Initialized Rawbin website project structure with core landing page components and UI configuration.
*   Designed the sticky header Navbar with clean styling and a styled Order Now CTA.
*   Replaced the placeholder brand logo with a high-definition version.

### Phase 2: Copywriting, Marquees & Layout Refinement
*   **Commits:** `5b9c471` to `c0f83c5`
*   Applied marketing copywriting revisions to the Hero section headline ("YOUR FRIDGE GOT SMART...") and sub-headlines.
*   Removed the generic Rawbin Community Impact counters to clean up user focus.
*   Added the infinite partner marquee logos strip with grayscale animations and hover properties.
*   Configured platform compatibility specs, Eight Sleep reference targets, and SEO strategy.

### Phase 3: Core Features: Calculator, Outline Icons & Router Fixing
*   **Commits:** `a67cae9` to `7cab1e2`
*   Built and deployed the dynamic **Return on Environment (ROE) Calculator** mapping CO2 savings.
*   Overhauled the icon system: replaced flat cartoon icons and emojis with global custom outline SVGs.
*   Upgraded the Technology page layout and specification badges.
*   Corrected Navbar page routing, anchor redirects, and fixed leading slash parameters.

### Phase 4: About Us, Testimonials horizontal Carousel & Press Redesigns
*   **Commits:** `2784869` to `6840a24`
*   Migrated About Us to a fully dedicated `/about-us` subpage detailing founder Anu Khandelwal's Twilio background, R&D timeline, and interactive LinkedIn feeds.
*   Redesigned the Testimonials section into a premium, dark-themed draggable horizontal infinite carousel. Migrated all 17 real testimonials and removed visual arrow noise.
*   Replaced the placeholder "As Seen In" block with a "Press Mentions" grid featuring clickable original NY Times, Red FM, and Ground Report logos on a warm cream background.

### Phase 5: Visuals, Science resource hub & Framer Motion
*   **Commits:** `2670ad2` to `ea4220c`
*   Replaced generic step images in the "How It Works" section with custom AI-generated step illustrations detailing real composting contexts.
*   Built the `/science` subpage with 12 detailed composting FAQs and added anchor science link buttons.
*   Integrated comprehensive Framer Motion animations (lilac scroll progress bar, spring transforms, breathing glowing CTA buttons).
*   Sprinkled the brand chocolate brown and lilac color accents across page headers, step borders, and specifications.
*   Transformed the Hero background left column into a glassmorphic frosted overlay utilizing linear mask gradients.
*   Refactored the Impact Section into a premium 3-column Results Panel dashboard.

### Phase 6: Specifications Slider, Theme Engine & Social Grids
*   **Commits:** `5a5231f` to `d341e16`
*   Created the "Why Rawbin vs. Traditional Composting" comparison table component.
*   Integrated the 3-column interactive Instagram Reels Grid under Compost Use Cases with video views, likes, comments metrics, and redirecting CTAs.
*   Designed the CSS variable-driven light/dark mode theme engine, complete with an head-script logic in the root layout to prevent unstyled flashes on refresh.
*   Fixed specifications slider double-borders and Safari/Chrome rendering corner overflow bugs.

### Phase 7: RAG Chatbot & PDF Report Integrations
*   **Commits:** `885d3f4` to `39d6877`
*   Mapped static PDF serving and wired Navbar/Footer dropdown links to open `PL.M.25-26.00078.pdf` in new tabs.
*   Integrated the Groq API completion route powered by Llama 3.1 8B (`llama-3.1-8b-instant`) to stay within Organization TPM limits on the free tier.
*   Trimmed RAG knowledge base context into `rawbin-knowledge.md` to guarantee fast completions.
*   Implemented server-side regex cleaning filters to remove bold asterisks and clean up lists into bullet points.
*   Configured the AskRawbin bubble component to track conversation history arrays and show typing bubbles.
